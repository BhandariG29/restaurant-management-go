...
func GetOrderItems() gin.HandlerFunc {
	return func(c *gin.Context){
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error":"error occured while listing all orderItems"})
		}
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	groupStage := bson.D{{"$group", bson.D{{"id", bson.D{{"order_id", "$order_id"}, {"table_id", "$table_id"}, {"table_number", "$table_number"}}}, {"payment_due", bson.D{{"$sum", "$amount"}}}, {"total_count", bson.D{{"$sum", 1}}}, {"order_items", bson.D{{"$push", "$$ROOT"}}} }}}
...
func UpdateOrderItem() gin.HandlerFunc {
	return func(c *gin.Context){
		...
		if err != nil {
			msg := fmt.Sprintf("orderItem update failed")
